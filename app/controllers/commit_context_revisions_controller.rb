...
# free.
#
# Deliberately NOT namespaced under a CommitContext module/controller_path:
# link_to_revision (core) and this plugin's own diff link both build
# `:controller => 'repositories'` without a leading slash, which Rails
# resolves relative to the *current* controller's namespace directory. A
# namespaced controller_path like "commit_context/revisions" would silently
# rewrite that to "commit_context/repositories" -- a route that doesn't
# exist -- breaking every revision link on this page.
class CommitContextRevisionsController < ApplicationController
